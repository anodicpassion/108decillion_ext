...
# def time_to_epoch(date, time_format="%Y-%m-%d %H:%M:%S"):
...
# def epoch_to_time(epoch, time_format="%Y-%m-%d %H:%M:%S"):
...
def epoch_to_time(
    ...
    if timezone == "Local":
        # Local timezone
        return datetime.datetime.fromtimestamp(epoch).strftime(time_format)
    else:
        tz = ZoneInfo(timezone)
        return datetime.datetime.fromtimestamp(epoch, tz).strftime(time_format)
