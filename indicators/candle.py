...
class OHLC:
    ...
    def __call__(self, *args, **kwargs) -> np.ndarray:
        return np.array([self.__datetime, round(self.__open, 2), round(self.__high, 2), round(self.__low, 2),
                         round(self.__close, 2), round(self.__volume, 2)],
                        dtype=np.float64)
...
class TOHLCV:
    def __init__(self, interval: str):
        if not isinstance(interval, int):
            raise TypeError("interval should be of type int")
        self.interval_epoch = int(interval)
    ...
    def transform(self, candle: np.ndarray) -> None:
        if candle is None or isinstance(candle, np.ndarray) is False:
            raise ValueError("candle should be non-empty array of type -> numpy.ndarray")
        ...
        if (isinstance(candle[0], float) or isinstance(candle[0], int)) and candle[0] > 0:
            current_epoch = candle[0]
            self.__datetime = current_epoch - (current_epoch % self.interval_epoch)
        else:
            raise TypeError("date should be in POSIX time")
